...
class SolarMasterCard extends LitElement {
  ...
  _translateWeather(state) {
    const dict = {
      'clear-night': 'Nuit Claire', 'cloudy': 'Nuageux', 'fog': 'Brouillard', 'partlycloudy': 'Partiellement Nuageux',
      ...
      <div class="page">
        ...
        <div class="cockpit-container">
            <svg viewBox="0 0 400 100" preserveAspectRatio="none" class="sun-svg-bg">
                <path d="M0,90 Q200,-20 400,90" fill="none" stroke="rgba(255,193,7,0.2)" stroke-width="2" stroke-dasharray="5"/>
            </svg>
  ...
  static styles = css`
    ...
    .sun-dot ha-icon { --mdc-icon-size: 20px; }
    
    .cockpit { position: relative; z-index: 2; display: flex; justify-content: space-between; align-items: center; height: 100%; }
    .side { flex: 1; }
    .mini-diag { background:rgba(0,0,0,0.5); padding:8px; border-radius:8px; margin:4px 0; border-left:4px solid #00f9f9; width:110px; backdrop-filter: blur(5px); }
    .mini-diag.r { border-left:none; border-right:4px solid #ffc107; margin-left:auto; text-align: right; }
    .m-l { font-size:8px; opacity:0.6; display:block; text-transform: uppercase; font-weight: 800; }
    .m-v { font-size:12px; font-weight:bold; }
    
    .big-val-titan { font-size:65px; font-weight:900; color:#ffc107; text-align:center; line-height:0.8; }
    .big-val-titan small { font-size: 20px; margin-left: 5px; }
    .obj-bar-titan { position: relative; height: 16px; background: rgba(255,255,255,0.1); border-radius: 4px; margin-top: 15px; overflow: hidden; border: 1px solid #ffc10744; }
    .obj-fill { height: 100%; background: #ffc107; }
    .obj-label { position: absolute; width: 100%; font-size: 9px; font-weight: 900; line-height: 16px; color: #000; text-align: center; }
    ...
    .hud-circle-giant .v { font-size: 20px; font-weight: 900; }
    ...
    .hud-n { font-size:10px; font-weight:bold; text-align:center; margin-top:5px; }
    .hud-sub-giant { font-size:12px; color:#00f9f9; font-weight:bold; text-align:center; }
    ...
    .eco-main-card { background: rgba(76,175,80,0.1); border: 1px solid #4caf5044; border-radius: 20px; padding: 20px; text-align: center; margin-bottom: 15px; }
    ...
    .e-stat span { font-size: 9px; opacity: 0.6; display: block; }
    .e-stat b { font-size: 18px; }
    .eco-details-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
    .e-detail { background: #ffffff05; padding: 12px; border-radius: 12px; text-align: center; }
    .e-detail span { font-size: 8px; opacity: 0.6; display: block; }

    .footer { display:flex; justify-content:space-around; padding-top:15px; border-top:1px solid #333; margin-top:auto; }
    .f-btn { cursor:pointer; opacity:0.5; font-size:11px; font-weight:bold; text-transform: uppercase; }
...
window.customCards.push({ type: "solar-master-card", name: "Solar Master Card", description: "v2.4.0 - Final Edition" });
