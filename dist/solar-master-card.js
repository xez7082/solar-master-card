...
class SolarMasterCard extends LitElement {
  ...
  _renderEco() {
    ...
    return html`<div class="page">
      <div class="eco-hero"><div class="e-val">${money.val}€</div><div class="e-sub">Économisé</div></div>
      <div class="eco-grid">
        <div class="e-card">JOUR: ${this._getVal(c.eco_day_euro).val}€</div>
        <div class="e-card">AN: ${this._getVal(c.eco_year_euro).val}€</div>
  ...
  static styles = css`
    ha-card { background: #000; color: white; border-radius: 20px; overflow: hidden; font-family: sans-serif; position: relative; }
    .overlay { height: 100%; display: flex; flex-direction: column; padding: 15px; background: linear-gradient(180deg, #131c25 0%, #000 100%); }
    .main-content { flex: 1; }
    .cockpit { display: flex; justify-content: space-between; align-items: center; margin-top: 20px; margin-bottom: 30px;}
    .side { flex: 1; }
    .side-title { font-size: 10px; color: #00f9f9; font-weight: bold; margin-bottom: 8px; text-transform: uppercase; }
    ...
    .hud-circle { width: 70px; height: 70px; border-radius: 50%; border: 2px solid; position: relative; display: flex; align-items: center; justify-content: center; }
    ...
    .hud-n { font-size: 10px; margin-top: 8px; text-align: center; opacity: 0.8; }
    .footer { display: flex; justify-content: space-around; padding-top: 20px; border-top: 1px solid #333; margin-top: auto; padding-bottom: 5px;}
    .f-btn { cursor: pointer; opacity: 0.5; font-size: 10px; font-weight: bold; padding: 10px; }
...
window.customCards.push({
  ...
  description: "Restauration Complète v1.6.5"
});
