    ...
    const schemas = {
      tab_solar: [
        ...
        ...[1, 2, 3, 4, 5, 6, 7, 8, 9].map(i => [
          { name: `d${i}_label`, label: `Label Diagnostic d${i}` },
          { name: `d${i}_entity`, label: `Entité Diagnostic d${i}`, selector: { entity: {} } }
        ]).flat()
      ],
...
class SolarMasterCard extends LitElement {
  ...
  _renderMiniDiag(i, side) {
    ...
    const d = this._smartGet(entityId);
    return html`
      ...
      <div class="page">
        ...
        <div class="cockpit-container">
          <div class="cockpit-side left">
            ...
            <div class="side-items">${[4, 5, 6].map(i => this._renderMiniDiag(i, 'l'))}</div>
          </div>
          ...
          <div class="cockpit-side right">
            ...
            <div class="side-items">${[7, 8, 9].map(i => this._renderMiniDiag(i, 'r'))}</div>
          </div>
        </div>
            ...
            return html`
              <div class="hud-item">
                <div class="hud-circle" style="border-color:${p.c}44">
                  ...
                  <div class="val-container"><span class="v" style="color:${p.c}">${Math.round(s.val)}</span></div>
                </div>
        ...
        <div class="diag-grid">
          ${[1, 2, 3].map(i => {
            ...
            return c[`d${i}_entity`] ? html`<div class="d-box"><span class="d-l">${c[`d${i}_label`]}</span><span class="d-v">${d.val}</span></div>` : '';
          })}
        </div>
      </div>`;
  }
      ...
      ${[1, 2, 3, 4].map(i => {
        ...
        return html`
          <div class="rack-card">
            ...
            <div class="v-meter">${[...Array(20)].map((_, idx) => html`<div class="v-seg ${parseInt(soc.val) > (idx * 5) ? 'on' : ''}"></div>`)}</div>
            <div class="r-f-grid-4">
              <div class="r-f-box cyan">${v.val}V</div><div class="r-f-box">${t.val}°</div><div class="r-f-box cyan">${a.val}A</div>
            </div>
          </div>`;
      })}
    </div>`;
  ...
  _renderEco() {
    ...
    return html`<div class="page">
      ...
      <div class="eco-stats-grid">
        <div class="stat-card"><span class="s-label">JOUR</span><span class="s-value green">${this._smartGet(c.eco_day_euro).val}€</span></div>
        <div class="stat-card"><span class="s-label">ANNUEL</span><span class="s-value yellow">${this._smartGet(c.eco_year_euro).val}€</span></div>
      </div>
    </div>`;
  }
  ...
  static styles = css`
    ...
    .custom-bg { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-size: cover; opacity: 0.2; }
    ...
    .t-badge { background: rgba(0,0,0,0.6); padding: 6px 10px; border-radius: 10px; font-size: 10px; border: 1px solid rgba(255,255,255,0.1); display: flex; align-items: center; gap: 5px; }
    ...
    .sun-mover ha-icon { --mdc-icon-size: 20px; color: #ffc107; }
    .sun-val { font-size: 8px; }
    .cockpit-container { display: flex; align-items: center; justify-content: space-between; margin-bottom: 25px; min-height: 100px; }
    .cockpit-side { flex: 1; }
    ...
    .mini-diag { background: rgba(255,255,255,0.05); padding: 4px 8px; border-radius: 6px; display: flex; flex-direction: column; border-left: 3px solid #00f9f9; margin-bottom: 4px; }
    .mini-diag.r { border-left: none; border-right: 3px solid #ffc107; text-align: right; }
    .m-l { font-size: 7px; opacity: 0.6; text-transform: uppercase; font-weight: bold; }
    .m-v { font-size: 11px; font-weight: 900; }
    .total-block { text-align: center; width: 140px; }
    ...
    .perf-fill { height: 100%; background: #ffc107; }
    .panels-row { display: flex; justify-content: space-around; margin-bottom: 20px; }
    .hud-circle { width: 70px; height: 70px; border-radius: 50%; border: 2px solid; position: relative; display: flex; align-items: center; justify-content: center; background: rgba(0,0,0,0.4); }
    ...
    .rack-card { background: rgba(255,255,255,0.05); padding: 10px; border-radius: 12px; margin-bottom: 8px; border-left: 4px solid #4caf50; }
    .v-meter { display: flex; gap: 1px; height: 5px; margin: 8px 0; }
    .v-seg { flex: 1; background: rgba(255,255,255,0.1); }
    .v-seg.on { background: #4caf50; }
    .r-f-grid-4 { display: grid; grid-template-columns: repeat(3, 1fr); gap: 5px; }
    .r-f-box { background: #000; padding: 4px; border-radius: 4px; font-size: 9px; text-align: center; }
    .cyan { color: #00f9f9; }
    .eco-hero { background: rgba(76,175,80,0.1); padding: 15px; border-radius: 15px; text-align: center; margin-bottom: 15px; }
    .e-big { font-size: 38px; font-weight: 900; color: #4caf50; }
    .e-bar-wrap { height: 6px; background: rgba(255,255,255,0.1); border-radius: 10px; margin: 10px 0; overflow: hidden; }
    .e-bar-fill { height: 100%; background: #4caf50; }
    .eco-stats-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
    .stat-card { background: rgba(0,0,0,0.5); padding: 10px; border-radius: 12px; text-align: center; }
    .s-value { font-size: 14px; font-weight: bold; }
    .yellow { color: #ffc107; }
    .empty { visibility: hidden; height: 30px; }
