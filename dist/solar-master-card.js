    ...
    const schemas = {
      tab_solar: [
        ...
        ...[1, 2, 3, 4].map(i => [
          ...
          { name: `p${i}_sub`, label: `Info Sous Cercle P${i} (ex: V ou A)`, selector: { entity: {} } }
        ]).flat(),
        ...[4, 5, 6, 7, 8, 9].map(i => [
      ...
      tab_batt: [...[1, 2, 3, 4].map(i => [
        ...
        { name: `b${i}_cap`, label: `Capacité/Santé ${i}`, selector: { entity: {} } }
      ]).flat()],
      tab_eco: [
...
class SolarMasterCard extends LitElement {
  ...
  _renderSolar() {
    ...
    return html`
      <div class="page">
        <div class="cockpit">
          <div class="side">${c.title_left}${[4, 5, 6].map(i => this._renderMiniDiag(i, 'l'))}</div>
          <div class="center"><div class="big-val">${prod.val}<small>W</small></div></div>
          <div class="side right-align">${c.title_right}${[7, 8, 9].map(i => this._renderMiniDiag(i, 'r'))}</div>
        </div>
            ...
            return html`
              <div class="hud-item">
                <div class="hud-circle" style="border-color:${colors[i-1]}44">
                  ...
                  <div class="v" style="color:${colors[i-1]}">${Math.round(s.val)}<small>W</small></div>
                </div>
                <div class="hud-n">${c[`p${i}_name`]}</div>
      ...
      ${[1, 2, 3, 4].map(i => {
        ...
        return html`
          <div class="rack">
            ...
            <div class="r-grid">
              <div class="r-item"><span>VOLTS</span><br><b>${this._getVal(c[`b${i}_v`]).val}V</b></div>
              <div class="r-item"><span>AMPS</span><br><b>${this._getVal(c[`b${i}_a`]).val}A</b></div>
              <div class="r-item"><span>TEMP</span><br><b>${this._getVal(c[`b${i}_temp`]).val}°</b></div>
              <div class="r-item"><span>SANTÉ</span><br><b>${cap.val}${cap.unit}</b></div>
            </div>
          </div>`;
      })}
    </div>`;
  ...
  _renderEco() {
    ...
    return html`<div class="page">
      <div class="eco-hero"><div class="e-val">${money.val}€</div><span>Économies Totales</span></div>
      <div class="eco-grid">
        ...
        <div class="e-card"><span>EDF/kWh</span><br><b>${this._getVal(c.kwh_price).val}€</b></div>
  ...
  static styles = css`
    ...
    .side { flex:1; font-size:10px; font-weight:bold; color:#00f9f9; }
    .right-align { text-align:right; color:#ffc107; }
    .mini-diag { background:rgba(255,255,255,0.05); padding:8px; border-radius:8px; margin:5px 0; border-left:4px solid #00f9f9; }
    .mini-diag.r { border-left:none; border-right:4px solid #ffc107; }
    ...
    .hud-circle { width:65px; height:65px; border-radius:50%; border:2px solid; position:relative; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.5); }
    ...
    .rack { background:rgba(255,255,255,0.05); padding:15px; border-radius:15px; margin-bottom:10px; border-left:4px solid #4caf50; }
    .r-h { display:flex; justify-content:space-between; font-weight:bold; }
    .soc-pct { color:#4caf50; font-size:16px; }
    .v-meter { display:flex; gap:2px; height:6px; margin:10px 0; }
    .v-seg { flex:1; background:#222; }
    .v-seg.on { background:#4caf50; box-shadow:0 0 5px #4caf50; }
    .r-grid { display:grid; grid-template-columns:repeat(4, 1fr); text-align:center; font-size:10px; }
    .eco-hero { text-align:center; padding:20px; background:rgba(76,175,80,0.1); border-radius:20px; margin-bottom:15px; }
    .e-val { font-size:40px; font-weight:bold; color:#4caf50; }
    .eco-grid { display:grid; grid-template-columns:1fr 1fr; gap:10px; }
    .e-card { background:rgba(255,255,255,0.05); padding:10px; border-radius:10px; text-align:center; }
...
window.customCards.push({ type: "solar-master-card", name: "Solar Master Card", description: "v1.8.5 - Correctif complet" });
