...
class SolarMasterCardEditor extends LitElement {
  ...
  render() {
    ...
    const schemas = {
      tab_solar: [
        ...
        { name: "bg_url", label: "URL Image de Fond (Optionnel)", selector: { text: {} } },
  ...
  static styles = css`.edit-tabs{display:flex;gap:5px;margin-bottom:15px}button{flex:1;padding:10px;background:#2c2c2c;color:#fff;border:none;border-radius:5px;cursor:pointer;font-weight:bold}button.active{background:#ffc107;color:#000}`;
}
...
class SolarMasterCard extends LitElement {
      ...
      ${[1,2,3,4].map(i => c[`b${i}_s`] ? html`
        <div class="rack-card">
          ...
          <div class="r-f-grid-4">
            <div class="r-f-box cyan">${this._get(c[`b${i}_v`])}V</div>
            ...
            <div class="r-f-box cyan">${this._get(c[`b${i}_a`])}A</div>
          </div>
        </div>` : '')}
    </div>`;
  }
  ...
  _renderEco() {
    ...
    return html`<div class="page">
      <div class="eco-hero">
        ...
        <div class="e-bar-wrap"><div class="e-bar-fill" style="width:${Math.min(100, (current/(c.eco_target || 1))*100)}%"></div></div>
  ...
  static styles = css`
    ...
    .custom-bg { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-size: cover; background-position: center; z-index: 0; transition: 0.5s; }
    .overlay { 
      height: 100%; display: flex; flex-direction: column; padding: 15px; position: relative; z-index: 1;
      background: radial-gradient(circle at 50% 0%, rgba(26, 42, 58, 0.7) 0%, rgba(0,0,0,0.8) 80%);
    }
    .top-nav { display: flex; gap: 8px; margin-bottom: 20px; }
    .t-badge { background: rgba(0,0,0,0.5); padding: 8px 12px; border-radius: 12px; font-size: 11px; font-weight: bold; border: 1px solid rgba(255,255,255,0.1); backdrop-filter: blur(5px); display: flex; align-items: center; gap: 5px; }
    ...
    .panels-row { display: flex; justify-content: space-around; }
    ...
    .f-btn { opacity: 0.5; cursor: pointer; display: flex; flex-direction: column; align-items: center; font-size: 9px; font-weight: bold; }
    .f-btn.active { opacity: 1; color: #ffc107; }
    .rack-card { background: rgba(0,0,0,0.5); padding: 12px; border-radius: 18px; margin-bottom: 10px; border-left: 4px solid #4caf50; }
    .v-meter { display: flex; gap: 1.5px; height: 8px; }
    .v-seg { flex: 1; background: rgba(255,255,255,0.05); }
    .v-seg.on { background: #4caf50; }
    .r-f-grid-4 { display: grid; grid-template-columns: repeat(4, 1fr); gap: 5px; margin-top: 10px; }
    .r-f-box { background: rgba(0,0,0,0.8); padding: 6px 2px; border-radius: 8px; font-size: 10px; text-align: center; border: 1px solid #333; }
